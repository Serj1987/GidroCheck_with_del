...
class TableDateWindow(Screen):
    ...
    def add_date_table(self):
        ...
        self.cur.execute(
            "SELECT number_detail, name_detail, quantity_detail, date, note, tag FROM sent WHERE date = %s ORDER BY "
            "date DESC",
            (self.date,))
        self.rows = self.cur.fetchall()
        ...
        self.cur2 = self.con.cursor()
        self.cur2.execute(
            "SELECT number_detail, name_detail, quantity_detail, date, note, tag FROM details WHERE date = %s ORDER "
            "BY date DESC",
            (self.date,))
        self.rows2 = self.cur2.fetchall()
...
class DeleteString(Screen):
    ...
    def delete_string_table(self):
        ...
        self.cur.execute("SELECT id, number_detail, name_detail, quantity_detail, date, note, tag FROM details UNION "
                         "SELECT id, number_detail, name_detail, quantity_detail, date, note, tag FROM sent ORDER BY "
                         "date "
                         " DESC")
        self.rows = self.cur.fetchall()
    ...
    def on_del_press_send(self):
        ...
        q = req[0][0]
        delete_query = '''DELETE FROM sent WHERE id = %s '''
        cur.execute(delete_query, (q,))
        con.commit()
